  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .cat-item {
      ...
      gap: 8px; cursor: pointer; width: 100%; max-width: 80px;
    }
    ...
    .cat-item:hover .cat-icon-wrap {
      border-color: rgba(0,229,160,0.4); transform: translateY(-3px);
    }
    ...
    @media (max-width: 400px) {
      .category-grid {
        ...
        justify-content: flex-start; gap: 10px; padding-bottom: 6px; scrollbar-width: none;
      }
    ...
    .brand-label {
      ...
      font-size: 9px; color: #76b900; font-weight: 700;
    ...
    .brand-item {
      display: flex; align-items: center; justify-content: center;
      padding: 6px 32px; border-right: 1px solid rgba(255,255,255,0.06); cursor: default;
    }
    ...
    .offer-badge {
      margin: 8px 10px 10px; display: inline-flex; align-items: center; gap: 4px;
...
<div class="cat-section">
  ...
  <div class="product-grid" id="productGrid">
    ...
    <a href="/product/{{ $product->id }}" class="product-card" data-index="{{ $index }}">
      <div class="product-img">
        ...
        @else
          <div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;color:var(--text-muted);font-size:13px;">
            No Image
          </div>
        @endif
      </div>
  ...
  <!-- SHOW MORE BUTTON (hanya 1, id unik) -->
...
<script>
  ...
  (function () {
    ...
    function initProductGrid() {
      ...
      const anyHidden = allCards.some((_, i) => i >= visibleUntil);
    ...
    function toggleProducts() {
      ...
      const anyHidden = allCards.some(card => card.style.display === 'none');

      if (!anyHidden) {
        // Semua tampil → collapse ke 2 baris
        visibleUntil = step;
        allCards.forEach((card, i) => {
          card.style.display = i < visibleUntil ? '' : 'none';
        });
        ...
        // Ada yang tersembunyi → tampilkan 2 baris lagi
        visibleUntil += step;
        allCards.forEach((card, i) => {
          card.style.display = i < visibleUntil ? '' : 'none';
        });

        const stillHidden = allCards.some(card => card.style.display === 'none');
        if (!stillHidden) {
