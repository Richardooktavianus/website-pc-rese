...
<style>
    ...
    .btn-detect:hover { background: rgba(0, 229, 160, 0.15); }
    ...
    .courier-label:hover { border-color: rgba(0, 229, 160, 0.3); }
    ...
    .courier-price { font-size: 11px; color: var(--accent); font-weight: 600; }
    .courier-eta   { font-size: 10px; color: var(--text-muted); }
    ...
    .payment-label:hover { border-color: rgba(0, 229, 160, 0.3); }
    ...
    .payment-info  { flex: 1; }
    .payment-name  { font-size: 14px; font-weight: 600; color: var(--text-primary); }
    ...
    .btn-checkout-main:active { transform: translateY(0); }
        ...
        <div class="checkout-layout">
            ...
            <div>
                ...
                <div class="co-card">
                    <div class="co-card-header">
                        <div class="co-card-icon">📝</div>
                        <span class="co-card-title">Catatan</span>
                    </div>
                    <div class="co-card-body">
                        <div class="form-group">
                            <label class="form-label">Catatan untuk Penjual</label>
            ...
            <div>
                <div class="summary-card">
                    ...
                    <div class="summary-body">
                        ...
                        {{-- Item list --}}
                        @if(count($checkoutItems) > 0)
                            @foreach($checkoutItems as $build)
                                        ...
                                        <div class="summary-item">
                                            <span class="summary-item-name">
                                                ...
                                                @if(isset($item['quantity']) && $item['quantity'] > 1)
                                                    <span style="font-size:11px;"> x{{ $item['quantity'] }}</span>
                                                @endif
                                            </span>
                                            <span class="summary-item-price">
                                                Rp {{ number_format($item['price'] * ($item['quantity'] ?? 1)) }}
                                            </span>
                                        </div>
                                    @endforeach
                                @endif
                        ...
                        @else
                            <div class="summary-item">
                                <span class="summary-item-name">Tidak ada item</span>
                            </div>
                        @endif

                        <hr class="summary-divider">

                        {{-- ✅ DISKON TIER OTOMATIS --}}
                        @if($discountAmount > 0)
                        <div class="discount-banner">
                            🎉 Diskon {{ $discountTier->label }} {{ $discountTier->discount }}% otomatis diterapkan!
                        </div>
                        @endif
                        ...
                        @if($discountAmount > 0)
                        <div class="summary-row-item">
                            <span class="summary-row-label" style="color:var(--accent);">
                                Diskon {{ $discountTier->label }} ({{ $discountTier->discount }}%)
                            </span>
                            <span class="summary-row-value green">
                        ...
                        <div class="summary-total">
                            ...
                            <span class="summary-total-price" id="totalDisplay">
                                Rp {{ number_format($subtotalAfterDiscount + 18000) }}
                            </span>
                        </div>
...
(function () {
    ...
    const subtotalAfterDiscount = {{ $subtotalAfterDiscount ?? 0 }};
    let ongkir = 18000;
    ...
    // Deteksi lokasi dengan OpenStreetMap
    window.detectLocation = function () {
        ...
        navigator.geolocation.getCurrentPosition(pos => {
            ...
            mapBox.innerHTML = `<iframe
                ...
                style="width:100%;height:200px;border:none;border-radius:10px;"
                loading="lazy">
            </iframe>`;
        }, () => {
    ...
    // Init map default (Bandung)
    (function initMap() {
        const lat = -6.9147, lng = 107.6098, pad = 0.05;
        document.getElementById('mapBox').innerHTML = `<iframe
            src="https://www.openstreetmap.org/export/embed.html?bbox=${lng-pad},${lat-pad},${lng+pad},${lat+pad}&layer=mapnik&marker=${lat},${lng}"
            style="width:100%;height:200px;border:none;border-radius:10px;"
            loading="lazy">
        </iframe>`;
